<?php
    $con=new mysqli("localhost","root","") or die("Error in connecting to database".$con->error());
    $con->select_db("rpgdb") or die("Error in connecting to rpgdb".$con->error());
    ...
    if(isset($_POST['sbtn'])){
        ...
        if(in_array($photoActualExt,$match) && in_array($sigActualExt,$match)){
                if($photoSize<=500000 && $sigSize<=500000){
